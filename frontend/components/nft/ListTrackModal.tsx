...
export function ListTrackModal({ track, onClose, onListed }: { track: { tokenId: number; name: string; artist: string }; onClose: () => void; onListed: () => void }) {
  ...
  const submit = async () => { if (!isConnected || !address) { await connect(); return; } setSubmitting(true); setError(''); try {
    const buildResponse = await fetch(`${API_URL}/api/listing/build`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ seller: address, tokenId: track.tokenId, price }) });
    ...
    const submitResponse = await fetch(`${API_URL}/api/listing`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ signedTxXDR, seller: address, tokenId: track.tokenId, price }) });
